    ...
    stage('Verify') {
      steps {
        sh '''
          ...
          check_contains 'direct frontend build marker' "name=\"workbench-build\" content=\"$BUILD_ID\"" "$FRONTEND_HTML"
          ...
          check_contains 'frontend proxy build marker' "name=\"workbench-build\" content=\"$BUILD_ID\"" "$PROXY_RESPONSE"
