...
test('mobile content overrides remove list-page overflow and center controls', () => {
  ...
  assert.match(styles, /\.mobile-card-list, \.mobile-accounting-list, \.todo-mobile-move\s*\{[^}]*display:\s*none/)
...
test('mobile dialogs are centered and accounting tables switch to cards', () => {
  ...
  assert.match(styles, /\.todo-card\s*\{[^}]*touch-action:\s*pan-y[^}]*user-select:\s*none/)
  assert.match(styles, /\.todo-drag-handle\s*\{[^}]*touch-action:\s*none/)
  assert.match(styles, /\.el-input__inner,[\s\S]*?\.el-input-number \.el-input__inner\s*\{[^}]*font-size:\s*16px !important/)
})
