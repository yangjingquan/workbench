<template>
  ...
  <div class="panel memo-editor">
    <div class="panel-header"><div><div class="panel-title">录入信息</div><div class="panel-subtitle">标题和内容都填写后才可以保存</div></div></div>
  ...
  <div class="memo-layout">
    ...
    <div class="panel memo-detail-panel">
      <div v-if="selectedMemo" class="memo-detail">
        <div class="panel-header"><div><div class="panel-title">{{ selectedMemo.title }}</div><div class="panel-subtitle">添加于 {{ formatDate(selectedMemo.created_at) }}</div></div><el-button @click="copyMemo">一键复制</el-button></div>
...
const form = reactive({ title: '', content: '' }); const keyword = ref(''); const memos = ref([]); const selectedId = ref(null); const saving = ref(false)
...
async function saveMemo() {
  ...
  saving.value = true
  try { await api.memo({ title: form.title.trim(), content: form.content.trim() }); form.title = ''; form.content = ''; await loadMemos(); ElMessage.success('备忘录已保存') } finally { saving.value = false }
}
