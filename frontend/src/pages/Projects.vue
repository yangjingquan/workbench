...
async function remove(project) { await ElMessageBox.confirm(`删除项目“${project.name}”后，里程碑、版本和提交记录会一并删除，工作记录、Todo、链接和备忘录会保留但解除项目归属。确定继续吗？`, '删除项目', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }); await api.deleteProject(project.id); if (app.currentProjectId === project.id) app.selectProject(null); await app.loadProjectContext(); ElMessage.success('项目已删除') }
